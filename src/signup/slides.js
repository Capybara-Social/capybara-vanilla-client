...
import icons from '../_generals/icons.json';
import { theme } from "../_generals/theme";
...
async function finishUser(){
	...
	finalUser["birthday"] = await (finalUser["birthday"]);
	finalUser["secret"] = await (finalUser["secret"]);
...
slides[1].children[1].children[0].oninput = ({target}) => {
...
slides[2].children[1].children[0].oninput = ({target}) => {
...
slides[2].children[1].children[1].oninput = ({target}) => {
